...
public class Prompt {
	...
	private void selectSoda(String input,Scanner scanner) {
		if (input.equals("1")) {
	...
	public static void main(String[] args) {
		...
		while (p.canBuy()) {
			...
			if(p.money < 900) {
				p.sodaLightHandler();
				p.printMenu(p.order);
				String lastQuestion = scanner.next();
				while(lastQuestion.equals("a")) {
					p.sodaLightHandler();
					p.printMenu("투입할 금액을 입력하세요. 0 입력시 취소하고 종료");
					int inputMoney = scanner.nextInt();
					p.money = p.money + inputMoney;
					if(p.money >= 900) {
						break;
					}
					if(inputMoney==0) {
						System.out.println(p.money+"원을 반환하고 종료합니다.");
						break;
					}
